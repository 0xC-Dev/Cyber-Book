...
> ⚠️ **Lab / exam use only.** Loading an unsigned DLL into LSASS is one of the loudest IOCs in Windows. **LSA Protection (RunAsPPL)** blocks the on-disk variant entirely. Do not use on real engagements.
